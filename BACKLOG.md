...
- [ ] **P1 / M** Split `NodeItem.cpp` (845 → 767 lines so far). ~~`effStyle/effFont/effPadding/...` helpers → `NodeStyleResolver`~~ done: extracted to `src/scene/NodeStyleResolver.{h,cpp}`, NodeItem callsites now resolve style once per method instead of recomputing through redundant accessors. Memoization keyed on `(theme, template, level)` is still pending. **Still to do:** extract `AddButtonOverlay` to its own file.
