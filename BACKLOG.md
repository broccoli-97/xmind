...
- [ ] **P2** Layout algorithms ignore collapse state: `measureSubtree`/`placeSubtree` walk `childNodes()` unconditionally, so a collapsed (hidden) subtree still reserves its full footprint — auto-layout leaves conspicuous gaps around collapsed branches. Fix would treat a collapsed node as a leaf during measure/place and carry hidden descendants along with their ancestor's offset.
